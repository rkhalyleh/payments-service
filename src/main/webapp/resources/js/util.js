...
var getEmptyRow = function(){
	return $('<tr class="orderpaymentsService-row-feilds"></tr>');
};
...
var getpaymentsServiceCombobox = function(name, w, cls) {
	var paymentsServiceCombo = getCombo(name, w, cls);
	
	$.getJSON("paymentsService", function(result) {
        $.each(result, function(index, item) {
            paymentsServiceCombo.append('<option value="' + item.id + '">' + item.name + '</option>');
        });
    });
	
	var cont = $("<div style='width: 88%'></div>")
	cont.append(paymentsServiceCombo);
	
	paymentsServiceCombo.combobox();
...
var addWidgetsToRow = function(widgets) {
	var row = $('<tr class="orderpaymentsService-row-feilds"></tr>');
